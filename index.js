...
// Challenge 4: total number of backers
const totalContributions = GAMES_JSON.reduce(
    (acc, game) => acc + game.backers,
    0
);

// Challenge 4: display contributions
...
// Challenge 4: total amount pledged
...
// Challenge 4: display total raised
...
function filterUnfundedOnly() {
    ...
    // Challenge 5: games that are NOT fully funded
    const unfundedGames = GAMES_JSON.filter(
        game => game.pledged < game.goal
    );

    // display unfunded games
    addGamesToPage(unfundedGames);

    // optional: for Secret Key checking
...
const descriptionText = `
    We've helped fund $${totalRaised.toLocaleString()} across 
    ${GAMES_JSON.length} games.
    Currently, ${numUnfunded} game${numUnfunded === 1 ? "" : "s"} 
    remain unfunded.
`;
